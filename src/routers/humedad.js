...
router.post('/insertar',
    [
        check('HUM_VALOR', 'La variable HUM_VALOR debe ser un real positivo.').isFloat({min: 1}),
    ...
    const { HUM_VALOR, ALM_ID } = req.body;
    const queryAlmId = 'SELECT COUNT(*) AS almacenes_cantidad FROM almacenes WHERE alm_id = ?;';
    const queryInsert = 'INSERT humedades (hum_valor, alm_id) VALUES (?, ?);';
    ...
    if (!existId) {
        ...
        await mysqlQuery(queryInsert, [HUM_VALOR, ALM_ID])
        .then((rows)=> {
